...
from casadi import SX,DM, inf, repmat, vertcat, collocation_points, \
                    substitute, Function, integrator, mtimes, vec, is_equal, SX
...
#        self.h_initial = vertcat(self.h_initial, x_c)
    ...
    def createQuadraticCost(self, par_dict):
        self.L = 0
        self.V = 0
        if not 'x_ref' in par_dict:
            par_dict['x_ref'] = DM.zeros(self.model.Nx)
        if not 'u_ref' in par_dict:
            par_dict['u_ref'] = DM.zeros(self.model.Nu)
            
        if 'Q' in par_dict:
            Q = par_dict['Q']
            self.L += mtimes(mtimes((self.model.x_sym-par_dict['x_ref']).T, Q),(self.model.x_sym-par_dict['x_ref']))
        
        if 'R' in par_dict:
            R = par_dict['R']
            self.L += mtimes(mtimes((self.model.u_sym-par_dict['u_ref']).T, R),(self.model.u_sym-par_dict['u_ref']))


        if 'Qv' in par_dict:
            Qv = par_dict['Qv']
            self.V += mtimes(mtimes((self.model.x_sym-par_dict['x_ref']).T, Qv),(self.model.x_sym-par_dict['x_ref']))
...
#==============================================================================
    ...
    def includeState(self, var, ode, x_0 = None, x_min = None, x_max=None, h_initial = None, x_0_sym = None, suppress = False):
        ...
        if x_0 == None and h_initial == None and not suppress:
