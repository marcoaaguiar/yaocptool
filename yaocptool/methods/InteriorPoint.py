...
class InteriorPoint:
    ...
    def __init__(self, problem, Ocp_solver_class, solver_options ={}, **kwargs):
        ...
        self.mu_0 = float(self.mu_0)
        self.beta = float(self.beta)
        if not 'mu_min' in kwargs:
            self.mu_min = self.mu_0/self.beta**4
        ...
        if 'integrator_type' in kwargs:
            solver_options['integrator_type'] = kwargs['integrator_type']
        if 'finite_elements' in kwargs:
            solver_options['finite_elements'] = kwargs['finite_elements']
...
#                    s_log = SX.sym('s_log',1)
#                    self.problem.includeState(x_log,self.model.x_sym[i]- self.problem.x_min[i],0) #-mu_sym*(log(+s_log - self.problem.x_min[i]) + log(self.problem.x_max[i] - s_log)), 0)
#                    self.problem.includeAlgebraic(s_log, s_log - self.model.x_sym[i])
#                    self.problem.V += -mu_sym*(log(+s_log - self.problem.x_min[i]) + log(self.problem.x_max[i] - s_log))
                    self.problem.L += mu_sym/(self.model.x_sym[i]- self.problem.x_min[i])
                    self.problem.L += mu_sym/(self.problem.x_max[i] - self.model.x_sym[i])
...
#                    self.problem.includeState(x_log, -mu_sym*log(+s_log - self.problem.x_min[i]), 0)
#                    self.problem.includeAlgebraic(s_log, s_log - self.model.x_sym[i])
#                    self.problem.V += x_log -mu_sym*log(+s_log - self.problem.x_min[i])
#                    self.m +=1
        ...
        if self.relax_control_bounds:
            for i in range(self.model.Nu):
                if self.problem.u_max[i] != inf:
                    self.problem.L += mu_sym/(-self.model.u_sym[i] + self.problem.u_max[i])
                    self.problem.u_max[i] = inf
                    self.m +=1

                if self.problem.u_min[i] != -inf:
                    self.problem.L += mu_sym/(+self.model.u_sym[i] - self.problem.u_min[i])
...
#            V_sol = self.ocp_solver.solve_raw(x0 = V_sol, p=[self.mu])
    ...
    def stepForward(self):
        self.mu = max(self.mu_min, self.mu/self.beta)
