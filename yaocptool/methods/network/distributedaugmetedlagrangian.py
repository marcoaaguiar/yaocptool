...
class DistributedAugmentedLagrangian(SolutionMethodInterface):
    def __init__(self, network, solution_method_class, solution_method_options=None, **kwargs):
        """
    ...
    def _include_exogenous_variables(self):
        for node in self.network.nodes:
            if node not in self.relax_dict:
                self.relax_dict[node] = {'u': [], 'y_relax': [], 'alg_relax_ind': [], 'eq_relax_ind': []}
            ...
            for p in self.network.graph.pred[node]:
                ...
                y_appr, y_appr_par = create_polynomial_approximation(node.problem.model.tau, y.shape[0],
                                                                     self.degree, name=y_appr_name)

                # register the added connection to the list of algebraic equations to be relaxed
                self.relax_dict[node]['alg_relax_ind'].extend(list(range(node.problem.model.alg.numel(),
                ...
                self.relax_dict[node]['y_relax'].extend(find_variables_indices_in_vector(u, node.problem.model.y_sym))
            ...
            for s in self.network.graph.succ[node]:
                ...
                u_appr, u_appr_par = create_polynomial_approximation(node.problem.model.tau, u.shape[0],
                                                                     self.degree, name=u_appr_name)

                self.relax_dict[node]['eq_relax_ind'].extend(list(range(node.problem.g_eq.numel(),
        ...
        if self._last_result.get(node) is None:
            if variable_type == 'y' and node.problem.y_guess is not None:
                ...
                return [[node.problem.u_guess[var_indices] for j in range(self.degree)] for i in
                        range(self.finite_elements)]
            return [[DM.zeros(len(var_indices)) for j in range(self.degree)] for i in range(self.finite_elements)]
    ...
    def solve(self):
        ...
        for node in self.network.nodes:
            node.solution_method = AugmentedLagrangian(node.problem, self.solution_method_class,
        ...
        for outer_it in range(self.max_iter_outer):
            ...
            for inner_it in range(self.max_iter_inner_first if outer_it == 0 and self.max_iter_inner_first is not None
                ...
                for node in sorted(self.network.nodes, key=lambda x: x.node_id):
                    ...
                    if self._last_result.get(node) is not None:
                        initial_guess_dict = self._last_result.get(node).raw_solution_dict
                    ...
                    # solve node problem
                    result = node.solution_method.solve(theta=node_theta[node],
            ...
            for node in sorted(self.network.nodes, key=lambda x: x.node_id):
                ...
                node_error[node] = node.solution_method._compute_new_nu_and_error(theta=theta_k, p=p_k,
                                                                                  raw_solution_dict=raw_solution_dict)
                print("Violation node: {} | error: {} | diff: {}".format(node.name, node_error[node],
            ...
            if max_error < self.abs_tol:
                print(
                    '=== Exiting: {} | Viol. Error: {} | Total time: {} ==='.format('Tolerance met', max_error,
                                                                                    time.time() - t_0))
                break
        else:
            print(
                '=== Exiting: {} | Iterations: {} | Viol. Error: {:e} | Total time: {} ==='.format(
                    'Max iteration reached', outer_it, float(max_error), time.time() - t_0))
    ...
    def plot_all_relaxations(self, result_list):
        for k, edge in enumerate(self.network.graph.edges):
            print(k)
            fig = plt.figure(k)
            y_index = find_variables_indices_in_vector(self.network.graph.edges[edge]['y'], edge[0].problem.model.y)
            u_index = find_variables_indices_in_vector(self.network.graph.edges[edge]['u'], edge[1].problem.model.u)
