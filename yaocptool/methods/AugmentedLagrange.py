...
from casadi import SX,DM, inf, repmat, vertcat, log, substitute, \
                    sum1, dot, collocation_points, vec, Function, linspace, MX, horzcat
...
class AugmentedLagrange(SolutionMethodsBase):
    ...
    def __init__(self, problem, Ocp_solver_class, solver_options ={}, **kwargs):
        ...
        self.degree = 4
        self.degree_control = self.degree
        ...
        if self.initialize:
            ...
            solver_options['degree'] = self.degree
            solver_options['degree_control'] = self.degree_control
...
#==============================================================================
    ...
    def relaxExternalAlgebraicEquations(self):
        ...
        self.problem.includeControl(z_without_con_z, u_max = self.problem.z_max[z_without_con_z_indeces], u_min = self.problem.z_min[z_without_con_z_indeces])
    ...
    def relaxConnectingEquations(self):
        ...
        self.problem.includeControl(self.model.con_z, u_max = self.problem.z_max[con_z_ind], u_min = self.problem.z_min[con_z_ind])
...
#==============================================================================
    ...
    def createNuInitialGuess(self, Nr = None):
        if Nr == None:
            Nr = self.Nr
        nu = self.createConstantTheta(constant = 0, dimension = Nr, degree = self.degree, finite_elements = self.finite_elements)
        return nu
        
    def createCalculateNewNu(self):
        X_var = MX.sym('X', self.model.Nx, (self.finite_elements+1))
        U_var = MX.sym('U', self.model.Nu*self.degree_control, (self.finite_elements))
        par = MX.sym('par')
        
        x_error = SX.sym('x_error')
        theta_dict = {}
        for i in range(self.finite_elements):
            theta_dict[i] = vec(MX.sym('theta_'+`i`, self.Nr,self.degree))
        theta_var = vertcat(*theta_dict.values())
        theta = theta_dict
...
#            x_0 = vertcat(X[k],DM.zeros(self.Nr))
            x_0 = vertcat(X[k],DM.zeros(1))
            ...
            func_nu = self.model.convertExprFromTauToTime(self.nu_pol, t_list[k], t_list[k+1])
...
#        return self.ocp_solver.plotSimulate(X, U, plot_list, n_of_sub_elements, p = self.mu, theta = self.nu, integrator_type = integrator_type)
    ...
    def simulate(self, X, U, sub_elements = 5, t_0 = None, t_f =None, p = [], theta = {}, integrator_type = 'implicit', time_division = 'linear'): 
        ...
        micro_X, micro_Y, micro_U, micro_t =  self.ocp_solver.simulate(X, U, sub_elements, t_0, t_f, par, nu_theta, integrator_type = integrator_type)
