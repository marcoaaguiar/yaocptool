...
class CollocationScheme(DiscretizationSchemeBase):
    ...
    def time_interpolation(self):
        tau_list = self.solution_method.collocation_points(self.degree,
                                                           with_zero=True)
    ...
    def _create_nlp_symbolic_variables(self, nlp):
        ...
        for k in range(self.finite_elements):
            ...
            for n in range(self.degree + 1):
                x_k.append(
                    nlp.create_variable('x_' + repr(k) + '_' + repr(n),
                                        self.model.n_x,
        ...
        for k in range(self.finite_elements):
            ...
            for n in range(self.degree):
                y_k.append(
                    nlp.create_variable('y_' + repr(k) + '_' + repr(n),
                                        self.model.n_y,
        ...
        for k in range(self.finite_elements):
            ...
            if self.model.n_u_par > 0:
                for n in range(self.degree_control):
                    u_k.append(
                        nlp.create_variable('u_' + repr(k) + '_' + repr(n),
                                            self.model.n_u,
        ...
        p_opt = nlp.create_variable('p_opt',
        ...
        for el in range(self.finite_elements):
            theta_opt.append(
                nlp.create_variable('theta_opt_' + str(el),
    ...
    def unpack_decision_variables(self,
                                  decision_variables,
        ...
        for k in range(self.finite_elements):
            ...
            for i in range(self.degree + 1):
                x_k.append(decision_variables[v_offset:v_offset +
                                              self.model.n_x])
        ...
        for k in range(self.finite_elements):
            ...
            for i in range(self.degree):
                y_k.append(decision_variables[v_offset:v_offset +
                                              self.model.n_y])
        ...
        for k in range(self.finite_elements):
            ...
            if self.model.n_u_par > 0:
                for i in range(self.degree_control):
                    u_k.append(decision_variables[v_offset:v_offset +
                                                  self.model.n_u])
        ...
        theta_opt = []
        for k in range(self.finite_elements):
            theta_opt.append(decision_variables[v_offset:v_offset +
                                                self.problem.n_theta_opt])
    ...
    def discretize(self, x_0=None, p=None, theta=None, last_u=None):
        ...
        # Create nlp object
        nlp = NonlinearOptimizationProblem(name='collocation_' +
                                           self.problem.name)
        ...
        for el in range(self.finite_elements):
            ...
            f_ode = Function('f_ode_' + repr(el),
                             self.model.all_sym, [dae_sys.ode],
                             self.model.all_sym_names(), ['ode'])
            f_alg = Function('f_alg_' + repr(el),
                             self.model.all_sym, [dae_sys.alg],
                             self.model.all_sym_names(), ['alg'])
        ...
        for el in range(self.finite_elements):
            ...
            if not el == 0:
                nlp.include_equality(results[el - 1]['x'][-1] -
                                     results[el]['x'][0])
            ...
            for i in range(self.problem.n_g_ineq):
                for collocation_point in range(
                        len(results[el]['g_ineq_' + str(i)])):
                    nlp.include_inequality(
                        results[el]['g_ineq_' + str(i)][collocation_point],
                        ub=0)

            for i in range(self.problem.n_g_eq):
                for collocation_point in range(
                        len(results[el]['g_eq_' + str(i)])):
                    nlp.include_equality(
        ...
        for el in time_dict:
            ...
            if self.solution_method.solution_class == 'direct':
                ...
                f_u = Function('f_u_pol', self.model.all_sym, [u_func],
                               self.model.all_sym_names(), ['f_u_pol'])
            ...
            y_pol, y_par = self.solution_method.create_variable_polynomial_approximation(
                self.model.n_y,
                self.degree,
                name=y_col_names,
                point_at_t0=False)
            ...
            for t in element_breakpoints:
                ...
                if self.solution_method.solution_class == 'direct':
                    ...
                    u_t = f_u(*self.model.put_values_in_all_sym_format(
        ...
        for el in range(self.finite_elements):
            ...
            for i in range(self.problem.n_g_ineq):
                ...
                if self.problem.time_g_ineq[i] == 'default':
                    time_dict[el][
                        'g_ineq_' +
                        str(i)] = self.time_interpolation_algebraics[el]
