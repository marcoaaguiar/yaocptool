...
class MultipleShootingScheme(DiscretizationSchemeBase):
    ...
    def discretize(self, x_0=None, p=None, theta=None, last_u=None):
        ...
        nlp = NonlinearOptimizationProblem(
            name="multiple_shooting_" + self.problem.name
        )
        ...
        for el in range(self.finite_elements):
            ...
            if (
                self.solution_method.solution_class == "direct"
                and self.solution_method.cost_as_a_sum
            ):
    ...
    def get_system_at_given_times(
        ...
        for el in time_dict:
            ...
            if self.solution_method.solution_class == "direct":
                f_u = Function("f_u_pol", [self.model.t, self.model.u_par], [u_func])
    ...
    def create_initial_guess_with_simulation(
        ...
        for el in range(self.finite_elements):
            ...
            dae_sys.convert_from_tau_to_time(
                t_k=self.time_breakpoints[el], t_kp1=self.time_breakpoints[el + 1]
            )
    ...
    def set_data_to_optimization_result_from_raw_data(
        ...
        if self.degree_control == 1:
            ...
            optimization_result.u_data["time"] = [
                [
                    t + self.delta_t * col
                    for col in self.solution_method.collocation_points(
                        self.degree_control
                    )
                ]
