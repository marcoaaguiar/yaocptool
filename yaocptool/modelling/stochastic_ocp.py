...
class StochasticOCP(OptimalControlProblem):
    ...
    def create_uncertain_parameter(
        self, name, mean, var, size=1, distribution="normal"
    ):
        par = self.model.create_parameter(name=name, size=size)

        self.set_parameter_as_uncertain_parameter(
            par, mean, var, distribution=distribution
        )
        return par
    ...
    def include_uncertain_parameter(self, par, mean, cov, distribution="normal"):
        ...
        self.set_parameter_as_uncertain_parameter(
            par, mean, cov, distribution=distribution
        )

    def set_parameter_as_uncertain_parameter(
        self, par, mean, cov, distribution="normal"
    ):
        par = vertcat(par)
        mean = vertcat(mean)
        cov = vertcat(cov)
        ...
        if not par.numel() == mean.numel():
            raise ValueError(
                'Size of "par" and "mean" differ. par.numel()={} '
