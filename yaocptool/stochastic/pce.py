...
class PCEConverter:
    ...
    def n_pol_parameters(self):
        n_pol_parameters = int(
            factorial(self.n_uncertain + self.pc_order)
    ...
    def _sample_parameters(self):
        ...
        mean = vertcat(
            self.socp.p_unc_mean, self.socp.uncertain_initial_conditions_mean
        )

        if self.socp.n_p_unc > 0 and self.socp.n_uncertain_initial_condition > 0:
            covariance = diagcat(
                self.socp.p_unc_cov, self.socp.uncertain_initial_conditions_cov
            )
            ...
            raise ValueError(
                "No uncertainties found n_p_unc = {}, "
                "n_uncertain_initial_condition={}".format(
                    self.socp.n_p_unc, self.socp.n_uncertain_initial_condition
                )
            )

        dist = (
            self.socp.p_unc_dist + self.socp.uncertain_initial_conditions_distribution
        )

        for d in dist:
            if not d == "normal":
                raise NotImplementedError(
                    'Distribution "{}" not implemented, only "normal" is available.'.format(
                        d
                    )
                )
        ...
        for s in range(n_samples):
            sampled_parameters[:, s] = (
                mean + mtimes(sampled_epsilon[:, s].T, chol(covariance)).T
            )
    ...
    def _create_cost_ode_of_sample(self, model_s):
        ...
        original_vars = vertcat(
            self.socp.model.x_sym,
            self.socp.model.y_sym,
            self.socp.model.u_sym,
            self.socp.model.p_sym,
            self.socp.model.theta_sym,
            self.socp.model.t_sym,
            self.socp.model.tau_sym,
            self.socp.model.x_0_sym,
        )
    ...
    def convert_socp_to_ocp_with_pce(self):
        ...
        self._construct_stochastic_objective(
            cost_list, exp_phi, ls_factor, self.problem
        )

        # uncertain constraints
        self._include_statistics_eqs_of_stochastic_variables(
            exp_phi, ls_factor, self.model, self.problem
        )
    ...
    def _include_statistics_eqs_of_stochastic_variables(
        ...
        for i in range(self.socp.n_g_stochastic):
            ...
            [
                ...
                stoch_ineq_var,
                _,
            ] = self._include_statistics_of_expression(
                var, name, exp_phi, ls_factor, model, problem
            )
            ...
            k_viol = sqrt(
                self.socp.g_stochastic_prob[i] / (1 - self.socp.g_stochastic_prob[i])
            )
    ...
    def _include_statistics_of_expression(
        self, expr, name, exp_phi, ls_factor, model, problem
    ):
        ...
        if self.variable_type == "theta":
            ...
            problem.include_time_equality(
                stochastic_var_par - mtimes(ls_factor, var_vector), when="end"
            )
            problem.include_time_equality(
                stochastic_var_mean - stochastic_var_par[0], when="end"
            )
            problem.include_time_equality(
                stochastic_var_var
                - mtimes((stochastic_var_par[1:] ** 2).T, exp_phi[1:]),
                when="end",
            )
    ...
    def _construct_stochastic_objective(self, cost_list, exp_phi, ls_factor, problem):
        # Exp(J) and Var(J)
        cost_pol_par = problem.create_optimization_parameter(
            "cost_pol_par", self.n_pol_parameters
        )
        ...
        problem.include_final_time_equality(
            var_cost - mtimes((cost_pol_par[1:] ** 2).T, exp_phi[1:])
        )
    ...
    def _create_problem(self, model, sampled_parameter):
        ...
        problem.p_opt = substitute(
            self.socp.p_opt, self.socp.get_p_without_p_unc(), problem.model.p_sym
        )
        problem.theta_opt = substitute(
            self.socp.theta_opt, self.socp.model.theta_sym, problem.model.theta_sym
        )
        ...
        if not self.socp.n_h_initial == self.socp.model.n_x:
            problem.h_initial = vertcat(
                problem.h_initial,
                substitute(
                    self.socp.h_initial[: self.socp.model.n_x], original_vars, new_vars
                ),
            )
        ...
        for i in range(self.socp.n_uncertain_initial_condition):
            ...
            problem.h_final = substitute(
                problem.h_final, x_ind_s, sampled_parameter[self.socp.n_p_unc + i, :].T
            )

            problem.g_eq = substitute(
                problem.g_eq, x_ind_s, sampled_parameter[self.socp.n_p_unc + i, :].T
            )
            problem.g_ineq = substitute(
                problem.g_ineq, x_ind_s, sampled_parameter[self.socp.n_p_unc + i, :].T
            )

        problem.last_u = self.socp.last_u

        problem.y_guess = (
            repmat(self.socp.y_guess, self.n_samples)
            if self.socp.y_guess is not None
            else None
        )
        problem.u_guess = self.socp.u_guess
        problem.x_0 = (
            repmat(vertcat(self.socp.x_0, 0), self.n_samples)
            if self.socp.x_0 is not None
            else None
        )
    ...
    def _get_expression_in_each_scenario(self, expr, model):
        ...
        for s in range(self.n_samples):
            x_s = model.x_sym[
                s * (self.socp.model.n_x + 1) : (s + 1) * (self.socp.model.n_x + 1)
            ][: self.socp.model.n_x]
            x_0_s = model.x_0_sym[
                s * (self.socp.model.n_x + 1) : (s + 1) * (self.socp.model.n_x + 1)
            ][: self.socp.model.n_x]
...
def get_ls_factor(n_uncertain, n_samples, pc_order, lamb=0.0):
    ...
    sobol_design = sobol_seq.i4_sobol_generate(
        n_uncertain, n_samples, ceil(np.log2(n_samples))
    )
    ...
    he8fcn = Function(
        "He8fcn", [x], [x ** 8 - 28 * x ** 6 + 210 * x ** 4 - 420 * x ** 2 + 105]
    )
    he9fcn = Function(
        "He9fcn", [x], [x ** 9 - 36 * x ** 7 + 378 * x ** 5 - 1260 * x ** 3 + 945 * x]
    )
    he10fcn = Function(
        "He10fcn",
        [x],
    ...
    exps = (
        p
        for p in product(range(pc_order + 1), repeat=n_uncertain)
        if sum(p) <= pc_order
    )
    next(exps)
    exps = list(exps)
    ...
    x_sample = np.random.multivariate_normal(
        np.zeros(n_uncertain), np.eye(n_uncertain), n_sample_expectation_vector
    )
