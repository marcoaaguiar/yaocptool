...
class ExtendedKalmanFilter(EstimatorAbstract):
    def __init__(self, model, **kwargs):
        ...
        if self.model.n_y > 0:
            if self.y_guess is not None:
                ...
                raise ValueError("If the model has algebraic it is necessary to provide a initial guess for it "
                                 "('y_guess').")
        else:
            self._x_aug = self.x_mean

        (self.augmented_model, self.a_aug_matrix_func,
         self.gamma_func, self.p_update_func) = self._create_augmented_model_and_p_update(model)

        # Data set
        self.dataset = DataSet(name=self.model.name)
        self.dataset.data['x']['size'] = self.model.n_x
        self.dataset.data['x']['names'] = ['ekf_' + self.model.x_sym[i].name() for i in range(self.model.n_x)]

        self.dataset.data['y']['size'] = self.model.n_y
        self.dataset.data['y']['names'] = ['ekf_' + self.model.y_sym[i].name() for i in range(self.model.n_y)]

        self.dataset.data['P']['size'] = self.model.n_x ** 2
        self.dataset.data['P']['names'] = ['ekf_P_' + str(i) + str(j) for i in range(self.model.n_x) for j in
                                           range(self.model.n_x)]

        self.dataset.data['P_y']['size'] = self.model.n_y ** 2
        self.dataset.data['P_y']['names'] = ['ekf_P_y_' + str(i) + str(j) for i in range(self.model.n_y) for j in
                                             range(self.model.n_y)]

        self.dataset.data['meas']['size'] = self.n_meas
        self.dataset.data['meas']['names'] = ['ekf_meas_' + str(i) for i in range(self.n_meas)]
    ...
    def estimate(self, t_k, y_k, u_k):
        ...
        sim_results = self.model.simulate(x_0=self.x_mean, t_0=self.t, t_f=self.t + self.t_s,
        ...
        # Covariance
        all_sym_values = self.model.put_values_in_all_sym_format(t=self.t, x=x_pred, y=y_pred,
                                                                 u=u_k, p=self.p, theta=self.theta)
        ...
        p_pred = self.p_update_func(transition_matrix, self._p_k_aug, gamma_matrix, self.r_v)

        # Obtain the matrix G (linearized measurement model)
        measurement_sym = self._get_measurement_from_prediction(self.model.x_sym, self.model.y_sym,
                                                                self.model.u_sym)
        dh_dx_aug_sym = jacobian(measurement_sym, vertcat(self.model.x_sym, self.model.y_sym))
        ...
        k_gain = mtimes(mtimes(p_pred, g_matrix.T), inv(mtimes(g_matrix, mtimes(p_pred, g_matrix.T)) + self.r_n))
    ...
    def _create_augmented_model_and_p_update(self, model):
        ...
        all_sym = list(self.model.all_sym[:6])

        # Mean
        a_func = Function('A_matrix', all_sym, [jacobian(self.model.ode, self.model.x_sym)])
        b_func = Function('B_matrix', all_sym, [jacobian(self.model.ode, self.model.y_sym)])
        c_func = Function('C_matrix', all_sym, [jacobian(self.model.alg, self.model.x_sym)])
        d_func = Function('D_matrix', all_sym, [jacobian(self.model.alg, self.model.y_sym)])
        ...
        p_update_func = Function('p_update',
