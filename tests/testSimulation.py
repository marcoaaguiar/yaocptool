...
x_0 = [1, 2]
sim_result = model.simulate(x_0, range(1, 10), 0.5, u=1., p=[1], theta=dict(zip(range(0, 9), [0] * 9)))
# sim_result.plot([{'x': 'all'}, {'y': 'all'}, {'u': 'all'}])

# Include data at the end of the sim_result
copy_of_sim_result = sim_result.get_copy()
sim_result2 = model.simulate(x_0, range(20, 30), 19, u=1., p=[1], theta=dict(zip(range(0, 10), [0] * 10)))
