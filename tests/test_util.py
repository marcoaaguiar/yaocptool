...
class TestUtil(unittest.TestCase):
    ...
    def test_find_variables_indices_in_vector(self):
        ...
        self.assertEqual(
            len(find_variables_indices_in_vector(SX.sym("wrong"), self.x_vector)), 0
        )
    ...
    def test_find_variables_in_vector_by_name(self):
        ...
        result = find_variables_in_vector_by_name(
            correct_var_name, self.x_vector, exact=True
        )
        self.assertEqual(len(result), 1)
        self.assertTrue(is_equal(result[-1], correct_var))
    ...
    def test_remove_variables_from_vector(self):
        ...
        remaining_ind = [
            ind
            for ind in range(self.x_vector.numel())
            if ind not in removed_variables_ind
        ]
        ...
        self.assertTrue(
            is_equal(
                remove_variables_from_vector(var_tor_remove, self.x_vector), correct_res
            )
        )

        # Test remove variable that it is not in the vector
        self.assertRaises(
            ValueError, remove_variables_from_vector, SX.sym("wrong_var"), self.x_vector
        )
    ...
    def test_remove_variables_from_vector_by_indices(self):
        ...
        self.assertTrue(
            is_equal(
                remove_variables_from_vector_by_indices(
                    removed_variables_ind, self.x_vector
                ),
                correct_res,
            )
        )
        ...
        self.assertTrue(
            is_equal(
                remove_variables_from_vector_by_indices([], self.x_vector),
                self.x_vector,
            )
        )

        # Test remove emptying a vector
        res = remove_variables_from_vector_by_indices(
            list(range(self.x_vector.numel())), self.x_vector
        )
        ...
        self.assertRaises(
            ValueError, remove_variables_from_vector_by_indices, [100], self.x_vector
        )
        self.assertRaises(
            ValueError, remove_variables_from_vector_by_indices, [-100], self.x_vector
        )
    ...
    def test_create_constant_theta(self):
        ...
        # Test 0 dimension
        res_dict = dict(
            zip(range(finite_elements), [constant * DM.ones(0, 1)] * finite_elements)
        )
    ...
    def test_create_polynomial_approximation(self):
        ...
        # Pol point at zero
        pol, par = create_polynomial_approximation(
            tau, size, degree, name="func", point_at_zero=True
        )
        ...
        size_zero = 0
        pol, par = create_polynomial_approximation(
            tau, size_zero, degree, name="func", point_at_zero=True
        )
        ...
        size_zero = 0
        pol, par = create_polynomial_approximation(
            tau, size_zero, degree_one, name="func", point_at_zero=False
        )
        ...
        size_zero = 0
        pol, par = create_polynomial_approximation(
            tau, size_zero, degree_one, name="func", point_at_zero=True
        )
