...
from casadi.casadi import mtimes
...
class TestSystemModel(TestCase):
    ...
    def test_n_x(self):
        assert self.ode_model.n_x == 3
        assert self.dae_model.n_x == 3
...
def test_remove_state(model):
    if model.n_x > 0:
        ...
        for ind in range(model.n_x):
            assert is_equal(model.x[ind], to_remove)

        # removed ode
        assert model.ode.numel() == n_ode_original - 1
        for ind in range(model.ode.numel()):
            assert is_equal(model.ode[ind], to_remove_eq)
...
def test_replace_variable_state(dae_model):
    # replace x
    original = dae_model.x
    replacement = SX.sym("new_x", original.numel())

    dae_model.replace_variable(original, replacement)

    assert is_equal(dae_model.ode, -replacement + dae_model.u, 30)
    assert is_equal(dae_model.alg, dae_model.y - replacement + dae_model.u**2,
                    30)
