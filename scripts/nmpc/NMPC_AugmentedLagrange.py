...
from yaocptool.nmpc.nmpc_scheme import NMPCScheme
...
plant.include_state(cost,
                    mtimes(mtimes(plant.x_sym.T, problem.Q), plant.x_sym) + mtimes(mtimes(plant.u_sym.T, problem.R),
                                                                                   plant.u_sym))
ocp_solver = AugmentedLagrangian(problem, IndirectMethod,
                                 {'degree': 1, },
                                 max_iter=1, mu_0=1e4, beta=10., mu_max=1e4, finite_elements=40, degree=2,
