...
class DoublePendulum(SystemModel):
    def __init__(self):
        ...
        M[1, 1] = J_2 + a_2 ** 2 * m_2
        c = SX(2, 1)
        c[0] = d_1 * dphi_1 + d_2 * (dphi_1 - dphi_2) + l_1 * m_2 * a_2 * sin(phi_1 - phi_2) * dphi_2 ** 2 - (
                    a_1 * m_1 + l_1 * m_2) * (g * sin(phi_1) + cos(phi_2) * u)
        c[1] = d_2 * (phi_2 - phi_1) - a_2 * m_2 * (
                    g * sin(phi_2) + l_1 * sin(phi_1 - phi_2) * dphi_1 ** 2 + cos(phi_2) * u)
...
class UpwardStabilization(OptimalControlProblem):
    def __init__(self, model, **kwargs):
        ...
        self.cost = {'Q': diag([10, 0.1, 1, 0.1, 1, 0.1]), 'R': 0.001, 'Qv': diag([1, 1, 1, 1, 1, 1]),
