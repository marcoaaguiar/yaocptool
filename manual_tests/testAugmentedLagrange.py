...
if AUG_LAGRANGIAN:
    ...
    solution_method = AugmentedLagrangian(problem, ocp_solver_class=ocp_solver_class,
...
if INDIRECT:
    ...
    problem = StabilizationMIMO2x2WithInequality(model, t_f=10)
    solution_method2 = IndirectMethod(problem,
                                      degree=degree,
                                      degree_control=degree_control,
                                      finite_elements=finite_elements,
                                      discretization_scheme=discretization_scheme)
...
if DIRECT:
    ...
    problem = StabilizationMIMO2x2WithInequality(model, t_f=10)
    solution_method2 = DirectMethod(problem,
                                    degree=degree,
                                    degree_control=degree_control,
                                    finite_elements=finite_elements,
                                    discretization_scheme=discretization_scheme)
    result2 = solution_method2.solve(p=[1], x_0=[2, 3, 0])
    figs = result2.plot([  # {'x': [0]},
        {'x': [0, 1]},
        {'u': [0]}
    ], figures=figs)
